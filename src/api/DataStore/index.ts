...
function defaultGetStore() {
    if (!defaultGetStoreFunc) {
        defaultGetStoreFunc = createStore(!IS_REPORTER ? "VelocityData" : "VelocityDataReporter", "VelocityStore");
    }
...
/**
 * Migrate a value name from an old key to a new key without losing data
 * This should only be used for some time and should NOT be used regularly
 *
 * @param oldKey
 * @param newKey
 * @param customStore Method to get a custom store. Use with caution (see the docs).
    ...
    return customStore("readwrite", store => {
        ...
        return new Promise((resolve, reject) => {
            getRequest.onsuccess = () => {
                if (getRequest.result) store.put(getRequest.result, newKey);
                store.delete(oldKey);
                resolve(promisifyRequest(store.transaction));
            };
