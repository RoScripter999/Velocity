...
import { extractAndLoadChunksLazy, filters, findAll, mapMangledModule, search } from "@webpack";
...
type Node = StringNode | RegexNode | FunctionNode | ObjectNode;
        ...
        switch (type) {
            case "testPatch": {
                ...
                if (Array.isArray(find)) {
                    // Canonicalize each string in the array
                    const canonicalizedFind = find.map(f => canonicalizeMatch(f));

                    // For arrays, manually search through all modules
                    const allMods = search("");
                    keys = Object.keys(allMods).filter(key => {
                        const src = String(allMods[key]);
                        return canonicalizedFind.every(str => src.includes(str));
                    });

                    if (keys.length !== 1)
                        return reply("Expected exactly one match with all array conditions, found " + keys.length);
                ...
                try {
                    ...
                    switch (type.replace("find", "").replace("Lazy", "")) {
                        ...
                        case "mapMangledModule":
                            const code = parsedArgs[0];
                            const mappersText = parsedArgs[1];

                            const mappers: Record<string, any> = {};
                            try {
                                const fn = new Function("filters", `return (${mappersText})`);
                                const obj = fn(filters);
                                Object.assign(mappers, obj);
                            } catch (err) {
                                return reply("Failed to parse mappers object: " + err);
                            }

                            try {
                                const result = mapMangledModule(code, mappers);

                                // Check if result is empty
                                if (!result || Object.keys(result).length === 0) {
                                    return reply("mapMangledModule found no results - check your filter code strings");
                                }

                                results = [result];
                            } catch (err) {
                                return reply("mapMangledModule failed: " + err);
                            }
                            break;
                        default:
