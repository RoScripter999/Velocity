...
import { Buttons, Forms, Icons, Menu, showToast, Text, TextArea, TextInput, Toasts, useEffect, useState } from "@webpack/common";
...
function PickerModal({ rootProps, close, prefilledContent }: { rootProps: ModalProps; close(): void; prefilledContent?: string; }) {
        ...
        return (
            <ModalRoot {...rootProps}>
                <ModalHeader className={cl("modal-header")}>
                    <Text variant="heading-lg/semibold">Loading...</Text>
    ...
    return (
        <ModalRoot {...rootProps}>
            <ModalHeader className={cl("modal-header")}>
                <Flex flexDirection="column" alignItems="flex-start" gap="2px" style={{ flex: 1, marginBottom: "7px" }}>
                    <Text variant="heading-lg/semibold">Message Tags</Text>
                    <Text variant="text-sm/normal" style={{ color: "var(--text-muted)" }}>
                        {messages.filter(m => m.command && m.content).length} tag{messages.filter(m => m.command && m.content).length !== 1 ? "s" : ""}
                    </Text>
                </Flex>
                <ModalCloseButton onClick={close} />
            </ModalHeader>
            ...
            <ModalFooter className={cl("footer")}>
                <Buttons.ButtonGroup direction="horizontal">
                    <Buttons.Button size="sm" text="Save & Close" onClick={saveMessages} fullWidth disabled={!canSave()} />
                    <Buttons.Button variant="secondary" size="sm" text="Import" onClick={importMessages} />
                    <Buttons.Button
                        variant="secondary"
                        size="sm"
