...
import { ChannelActions, ChannelStore, ConfirmModal, Menu, openModal, PermissionsBits, PermissionStore, showToast, Toasts, UserStore, VoiceStateStore } from "@webpack/common";
...
export default definePlugin({
    ...
    searchTerms: ["VoiceQueue", "voice queue", "vc queue"],
    ...
    contextMenus: {
        "channel-context": { render: patchChannelContextMenu, required: true }
    },
    ...
    flux: {
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceStateChangeEvent[]; }) {
            ...
            for (const state of voiceStates) {
                if (state.userId === UserStore.getCurrentUser().id && state.channelId && state.channelId !== queuedChannelId) {
                    queuedChannelId = null;
                    return;
                }
            }
