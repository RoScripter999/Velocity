...
import { Alerts, Buttons, Field, Forms, HelpMessage, Icons, LoadingIndicator, lodash, Parser, SearchableSelect, SearchBar, Select, Text, useCallback, useDeferredValue, useEffect, useMemo, useRef, UserStore, useState } from "@webpack/common";
...
function applySortMode(
    plugins: { plugin: typeof Plugins[keyof typeof Plugins]; isRequired: boolean; isNew: boolean; }[],
    sortBy: SortMode
): typeof plugins {
...
export default function PluginSettings() {
    ...
    const [pluginsData, requiredPluginsData] = useMemo(() => {
        ...
        const showApi = filters.status === SearchStatus.API_PLUGINS;
        for (const p of sortedPlugins) {
            if (p.hidden || (p.name.endsWith("API") && !showApi))
                continue;
                ...
                <>
                    <ErrorBoundary noop>
                        <SearchBar
                            ...
                            onChange={query => {
                                setVisibleCount(20);
                                updateFilters(prev => ({ ...prev, value: query }));
                            }}
                            onClear={() => {
                                setVisibleCount(20);
                                updateFilters(prev => ({ ...prev, value: "" }));
                            }}
                        />
